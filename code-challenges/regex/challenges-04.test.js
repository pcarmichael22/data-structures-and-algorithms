...
const isNum = (input) => {
    let regExTestOne = /\d/gm
    return regExTestOne.test(input);
};
...
const isCapitalized = (str) => {
    // Solution code here...
    let regExTestTwo = /[A-Z]\w+/g
    let ans = str.match(regExTestTwo);
    if (ans) {
        return ans;
    } else {
        return [];
...
const matchMonth = (input) => {
    // Solution code here...
    let regExTestFour = /\b[oO]ct(ober)?\b/g
    return regExTestFour.test(input);
};
...
const noPunctuation = str => {
    // Solution code here...
    let regExTestFive = /\d+\s|\w+\s/gi;
    return str.match(regExTestFive);
};
